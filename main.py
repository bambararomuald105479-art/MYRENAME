...
def sync_time():
    """
    Corrige le décalage d'horloge en patchant time.time()
    via l'API worldtimeapi.org — fonctionne sans permissions root.
    """
    try:
        resp = requests.get("https://worldtimeapi.org/api/ip", timeout=5)
        if resp.status_code == 200:
            server_time = resp.json().get("unixtime", None)
            if server_time:
                offset = server_time - time.time()
                if abs(offset) > 1:
                    original_time = time.time
                    time.time = lambda: original_time() + offset
                    print(f"⏰ Horloge corrigée: offset = {offset:.2f}s")
                else:
                    print("⏰ Horloge OK, pas de correction nécessaire")
    except Exception as e:
        print(f"⚠️ Sync horloge échouée (ignorée): {e}")
