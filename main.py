...
async def handle_start(client, message):
    ...
    await message.reply_text(
        ...
        "🗑️ /clearthumb — Supprimer le thumbnail sauvegardé\n\n"
...
async def handle_photo(client, message):
    ...
    # Supprime l'ancien thumbnail si existant
    if user_id in user_thumbnails:
        _cleanup(user_thumbnails[user_id])

    # Télécharge la photo (meilleure qualité)
...
async def handle_file(client, message):
    """Reçoit fichiers vidéo/document"""
    if not is_authorized(message.from_user.id):
        return
    ...
    if session.get('mode') == 'compress_waiting_file':
        ...
        thumb_status = "🖼️ Thumbnail: ✅" if user_thumbnails.get(user_id) else "🖼️ Thumbnail: ❌ (aucun)"
        await message.reply_text(
            f"📹 `{filename}`\n{thumb_status}\n\nChoisis la qualité:",
    ...
    thumb_status = "🖼️ Thumbnail: ✅" if user_thumbnails.get(user_id) else "🖼️ Thumbnail: ❌ (envoie une image pour en définir un)"
    await message.reply_text(
...
async def handle_callback(client, callback):
    ...
    if callback.data.startswith("compress_"):
        ...
        await callback.edit_message_text(
            f"⏳ Téléchargement depuis Telegram..."
        )
        ...
        await callback.edit_message_text(
            ...
            f"Le fichier sera envoyé directement depuis FFmpeg Micro."
        )
        ...
        # Envoie l'URL directement à Telegram — pas de transit par Railway
        try:
    ...
    original_msg = session['message']
    output_format = os.path.splitext(new_name)[1].lstrip('.') or 'mp4'
    ...
    await callback.edit_message_text("⏳ Traitement via FFmpeg Micro...")

    thumbnail_path = user_thumbnails.get(user_id)

    try:
        download_url = await ffmpeg_micro.process_video(
            file_path=local_path,
            api_keys=config.FFMPEG_MICRO_API_KEYS,
            key_index=next_ffmpeg_key(),
        ...
        await callback.edit_message_text(f"❌ Erreur traitement: {e}")
        _cleanup(local_path)
        return
    finally:
        _cleanup(local_path)

    # Envoie l'URL directement à Telegram
    try:
        await callback.edit_message_text("📤 Envoi vers Telegram...")
        if is_video:
