...
async def handle_callback(client, callback):
    ...
    # ── Extension différente : conversion via FFmpeg Micro ────────────────
    output_format = new_ext.lstrip('.')
    ...
    await callback.edit_message_text("⏳ Conversion via FFmpeg Micro...")
    try:
    ...
    # Télécharge le résultat converti pour ajouter le thumbnail
    final_path = f"/tmp/{user_id}_{new_name}"
    try:
        await callback.edit_message_text("⏳ Finalisation...")
        async with aiohttp.ClientSession() as dl_session:
            async with dl_session.get(download_url) as resp:
