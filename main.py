...
def is_session_expired(session: dict) -> bool:
    """Vérifie si la session a expiré (plus de 5 minutes)"""
...
async def handle_callback(client, callback):
    ...
    if not session:
        await callback.answer("⏰ Session expirée, recommence.", show_alert=True)
        return

    if is_session_expired(session):
        del user_sessions[user_id]
        await callback.answer("⏰ Session expirée (24h), recommence.", show_alert=True)
        return
